...
/**
 * Saga state row, one per order. The orchestrator updates this in the same transaction that
 * consumes each saga-reply event, so the row is the source of truth for "what step are we on" --
